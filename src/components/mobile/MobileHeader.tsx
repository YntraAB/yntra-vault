...
export default function MobileHeader({
  ...
  const { currentVault, filterCategory, setIsLocked, searchTerm, setSearchTerm } = useAppState();
          ...
          <div className="flex h-9 w-full items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)] px-3 focus-within:border-[var(--border-focus)]">
            ...
            {searchTerm && (
              <button
                ...
                className="flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-tertiary)] hover:text-[var(--text-primary)]"
              >
                <X size={14} />
              </button>
            )}
