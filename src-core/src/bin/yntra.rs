...
use yntra_vault_core::{
    ...
    cli::{
        ipc::{try_ipc_request, run_ipc_daemon, IpcRequest, IpcResponse},
        run_cmd::execute_secret_injection,
        tui::run_tui,
        git_credential::handle_git_credential,
        ssh_agent::run_ssh_agent_pipe,
        native_host::{run_native_host_loop, install_native_host_manifest},
        keychain::clear_session_token,
    },
};
...
enum ShamirAction {
    ...
    Recover {
        /// First share key (SL-SHARE1-...)
        #[arg(short = 'a', long)]
        share_a: String,
        /// Second share key (SL-SHARE2-...)
        #[arg(short = 'b', long)]
        share_b: String,
    },
}
...
async fn handle_sync(
    ...
    match &args.action {
        ...
        SyncAction::P2pListen { listen } => {
            ...
            run_p2p_sync_listener(listen, &subkeys.hmac_key, vault_path)?;
        }
        SyncAction::P2pConnect { server } => {
            let manager = open_vault(vault_path, password, keyfile)?;
            let subkeys = manager.get_subkeys()?;
            println!("{} Connecting to P2P Sync Peer at {}...", "→".blue().bold(), server);
            run_p2p_sync_client(server, &subkeys.hmac_key, vault_path)?;
            println!("{} P2P Sync Completed successfully!", "✓".green().bold());
        }
    }
    Ok(())
}
