...
use yntra_vault_core::vault::search::{generate_index_tokens, generate_trigrams, hash_trigram};
...
pub fn bench_vault_search(c: &mut Criterion) {
    ...
    group.bench_function("Generate Index Tokens (Text String)", |b| {
        b.iter(|| generate_index_tokens(black_box(text)));
    });
