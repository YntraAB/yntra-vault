...
pub fn generate_totp_at(config: &TotpConfig, timestamp: u64) -> crate::Result<TotpCode> {
    ...
    if config.period == 0 {
        return Err(VaultError::TotpError("TOTP period cannot be zero".into()));
    }
...
pub fn verify_totp(config: &TotpConfig, code: &str, window: Option<u32>) -> crate::Result<bool> {
    ...
    for i in 0..=window {
        let delta = (i as u64).saturating_mul(config.period);
    ...
    for param in query_part.split('&') {
        if let Some((key, value)) = param.split_once('=') {
            match key.to_lowercase().as_str() {
                ...
                "digits" => {
                    let d = value.parse().unwrap_or(6);
                    config.digits = if (1..=9).contains(&d) { d } else { 6 };
                }
                "period" => {
                    let p = value.parse().unwrap_or(30);
                    config.period = if p > 0 { p } else { 30 };
                }
                _ => {}
            }
        }
