...
Rebuilt 0.2.4 validation: 146 frontend/tooling tests and 296 optimized Rust tests passed (9 opt-in tests ignored). Windows, Linux, macOS and all four Android ABIs compiled successfully. The signed universal APK passed an Android 35 x86_64 emulator regression: the original 0.2.4 failure was reproduced, the replacement installed with the same version and signing identity without clearing app data, the vault-creation screen opened, and restart plus fresh app-data startup/restart succeeded. The live updater check passed for Android, Windows installer/portable/CLI and Linux. This is emulator evidence; a physical phone and a populated-vault upgrade were not tested.
