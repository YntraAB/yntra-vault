...
pub fn schedule_auto_clear(tx_id: u64, hash: u64, timeout_secs: u64) {
...
fn is_clipboard_matching_hash(expected_hash: u64) -> bool {
    #[cfg(target_os = "windows")]
    {
        if let Ok(current_hash) = get_windows_clipboard_hash() {
...
fn copy_macos_defended(text: &str) -> crate::Result<()> {
    ...
    let script = r#"
        ...
        on run argv
            ...
            item's setString:secretText forType:(current application's NSPasteboardTypeString)
...
mod tests {
    ...
    fn test_windows_clipboard_hash_matching() {
        ...
        let expected_hash = compute_text_hash(secret);
        let result = copy_to_clipboard_defended(secret, true, None);
        assert!(result.is_ok());

        assert!(is_clipboard_matching_hash(expected_hash));
        assert!(!is_clipboard_matching_hash(expected_hash ^ 0xDEADBEEF));
    ...
    fn test_schedule_auto_clear_execution() {
        let _guard = CLIPBOARD_TEST_LOCK.lock().unwrap();
        let tx_id = CLIPBOARD_TX_COUNTER.fetch_add(1, Ordering::SeqCst);
        let hash = compute_text_hash("temp_secret");
        if let Ok(mut lock) = ACTIVE_CLIPBOARD_STATE.lock() {
            *lock = Some(ClipboardState { tx_id, hash });
        }

        // Schedule clear after 1 second
        schedule_auto_clear(tx_id, hash, 1);
        std::thread::sleep(Duration::from_millis(1500));

        // State must be cleared
        if let Ok(lock) = ACTIVE_CLIPBOARD_STATE.lock() {
            assert_eq!(*lock, None);
        }
    }
}
