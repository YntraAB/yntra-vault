...
export function AttachmentPreviewModal({
  ...
  const [contrastBg, setContrastBg] = useState(false);
  ...
  }, [open, closedLocally, handleClose]);
  ...
  const imageUrl = useMemo(() => {
    ...
    const blob = new Blob([data as Uint8Array<ArrayBuffer>], { type: attachment?.mime_type || attachment?.mimeType || 'image/png' });
    return URL.createObjectURL(blob);
  }, [category, data, attachment]);

  // Revoke object URL on unmount / change in background without blocking main thread
  useEffect(() => {
    return () => {
      if (imageUrl) {
  ...
  // Decoded text content (capped at 2MB to prevent UI lockup on massive text files)
  const textContent = useMemo(() => {
    ...
    try {
      ...
      return data.length > maxBytes
        ? text + '\n\n[... Preview truncated at 2 MB. Download file to view full content ...]'
        : text;
    } catch {
  ...
  // Parsed ZIP file entries (fast metadata-only inspection without inflating file bodies)
  ...
  if (!open || !attachment || closedLocally) {
    return null;
  }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 select-none"
      onClick={handleClose}
      onPointerDown={(e) => {
    ...
    >
      ...
      >
        ...
        <div
          className={`flex-1 min-h-0 overflow-auto p-4 transition-colors duration-200 ${
            category === 'image' && contrastBg
              ? resolvedTheme === 'dark'
                ? 'bg-zinc-200'
                : 'bg-zinc-900'
              : 'bg-[var(--bg-base)]/50'
        ...
        >
          ...
          {!data && (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-center select-none">
              <Loader2 size={26} className="animate-spin text-[var(--text-secondary)]" />
          ...
          {data && category === 'image' && imageUrl && (
            <div className="flex flex-col h-full items-center justify-center relative group">
              <div className="absolute top-2 right-2 z-10 flex items-center gap-1 bg-[var(--bg-elevated)]/90 backdrop-blur-md rounded-md p-1 border border-[var(--border)] shadow-md select-none">
                ...
                <ActionTooltip content={contrastBg ? 'Default background' : 'Contrast background'}>
                  <button
                    type="button"
