...
/// Maximum section limits to prevent allocation bombs from corrupted files.
pub const MAX_KDF_PARAMS_LEN: usize = 64 * 1024; // 64 KB
pub const MAX_BIOMETRIC_BLOCK_LEN: usize = 64 * 1024; // 64 KB
pub const MAX_HARDWARE2FA_BLOCK_LEN: usize = 128 * 1024; // 128 KB
pub const MAX_PAYLOAD_LEN: usize = 512 * 1024 * 1024; // 512 MB

pub use yntra_crypto::kdf::KdfParams;
pub use yntra_crypto::biometric::EmbeddedBiometricHeader;
pub use yntra_crypto::hardware2fa::{EmbeddedHardware2FaHeader, Hardware2FaProtocol};
...
impl VaultFile {
    ...
    pub fn from_bytes(data: &[u8]) -> crate::Result<Self> {
        ...
        if kdf_len > MAX_KDF_PARAMS_LEN || kdf_len > remaining {
            return Err(VaultError::InvalidFormat(
                "KDF params length exceeds maximum allowed or available bytes".into(),
            ));
        }
        ...
        let biometric = if flags & FLAG_HAS_BIOMETRIC != 0 {
            ...
            let remaining = data.len().saturating_sub(cursor.position() as usize);
            if bio_len > MAX_BIOMETRIC_BLOCK_LEN || bio_len > remaining {
                return Err(VaultError::InvalidFormat(
                    "Biometric block length exceeds maximum allowed or available bytes".into(),
                ));
            }
        ...
        let hardware2fa = if flags & FLAG_HAS_HARDWARE_2FA != 0 {
            ...
            let remaining = data.len().saturating_sub(cursor.position() as usize);
            if hw_len > MAX_HARDWARE2FA_BLOCK_LEN || hw_len > remaining {
                return Err(VaultError::InvalidFormat(
                    "Hardware 2FA block length exceeds maximum allowed or available bytes".into(),
                ));
            }
        ...
        if payload_len_u64 > (MAX_PAYLOAD_LEN as u64) || payload_len_u64 > remaining {
            return Err(VaultError::InvalidFormat(
                "Payload length exceeds maximum allowed or available bytes".into(),
            ));
        }
